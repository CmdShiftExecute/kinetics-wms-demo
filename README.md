...
| Route | Description |
|---|---|
| `/` | Overview: stock value and utilisation lead, followed by daily storage cost, ageing exposure and reorder position, then one block per question. The run-out list holds every group at or under its reorder point plus every group projected to run out within 60 days. |
...
13. `bun run check` chains `data`, `reconcile`, `stable`, `typecheck`, `lint`, `build` and `contrast`, in that order.
...
`bun run stable` is the byte-stability gate. It hashes every file under `public/data`, re-runs the generator and the reconciliation, hashes again, and fails if a single file differs, is added or is removed. It holds because no published file carries the clock time of the run any more: `generatedAt` and the reconciliation's `checkedAt` are both set from the one fixed `DATA_AS_OF` constant in `scripts/generate_demo_data.ts`, the same instant the pages already show as the data's as-of time. The gate runs inside `bun run check` immediately after `reconcile`, so a check before a commit never dirties the tree and a diff under `public/data` is always a real change.
...
The overview preserves the five warehouse questions with a stronger reading hierarchy and a source-backed management-attention row. Three local themes and circular suite menus follow the finalized MIS contract. Theme preference is per origin under `halvard-wms-theme`; ports do not synchronize preferences. Calculator vertical browsing requires **Open vertical**; retained drafts, exact CBM arithmetic and reset are unchanged.

See [Refinement preservation and verification](docs/REFINEMENT.md) for the before/after map, review gates and deployment boundary. Run `WMS_BASE=<preview-origin> WMS_OUT=<evidence-directory> node scripts/refinement.mjs` for theme, menu, all-group, narrow-screen and anchor checks. Set `WMS_BROWSER=firefox` for the same gate in Firefox. This supplements the existing numerical and interaction gates.
