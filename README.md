...
The four-month projection. Each vertical's month-end CBM for the next four months starts from the opening quantity, subtracts forecast issues, adds any in-transit arrival due that month, and adds a replenishment to max stock landing one lead time after the month in which the reorder point is crossed (for a group already below its reorder point on the stock date, that replenishment lands one lead time from the stock date itself). Lead times are mapped to months against the real day boundaries of the projection months, never a flat 30 days. Issues can never take a balance below zero, and an arrival lands on that floored balance rather than against a shortfall. The reconciliation script re-implements this rule from the definition and replays it from the group files, so the published projection cannot drift from the stated rule.
